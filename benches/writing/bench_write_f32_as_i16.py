...
def benchmark_i16_as_f32_8000(runner):
    ...
    runner.timeit(
        "TorchAudio-f32-as-i16-Sox-8000",
        stmt=
        'for f, o in zip(DATA, OUT_DATA): data=torchaudio.load(f)[0].type(torch.FloatTensor);os.makedirs(Path(o).parents[0], exist_ok=True);torchaudio.save(o,  data, 8000, encoding="PCM_S", bits_per_sample=16)',
        setup=
    ...
    runner.timeit(
        "TorchAudio-f32-as-i16-Soundfile-8000",
        stmt=
        'for f, o in zip(DATA, OUT_DATA): data=torchaudio.load(f)[0].type(torch.FloatTensor);os.makedirs(Path(o).parents[0], exist_ok=True);torchaudio.save(o, data, 8000, encoding="PCM_S", bits_per_sample=16)',
        setup=
    ...
    # )
    runner.timeit(
        "Soundfile-f32-as-i16-8000",
        stmt=
        'for f, o in zip(DATA, OUT_DATA): data=soundfile.read(f, dtype=np.float32)[0];os.makedirs(Path(o).parents[0], exist_ok=True);soundfile.write(o, data, 8000, subtype="PCM_16")',
        setup=
...
def benchmark_i16_as_f32_16000(runner):
    ...
    runner.timeit(
        "TorchAudio-f32-as-i16-Sox-16000",
        stmt=
        'for f, o in zip(DATA, OUT_DATA): data=torchaudio.load(f)[0].type(torch.FloatTensor);os.makedirs(Path(o).parents[0], exist_ok=True);torchaudio.save(o,  data, 16000, encoding="PCM_S", bits_per_sample=16)',
        setup=
        f"import os; from pathlib import Path;import torchaudio;torchaudio.set_audio_backend('sox_io');import torch; DATA = {DATA};OUT_DATA={OUT_DATA}"
    )
    runner.timeit(
        "TorchAudio-f32-as-i16-Soundfile-16000",
        stmt=
        'for f, o in zip(DATA, OUT_DATA): data=torchaudio.load(f)[0].type(torch.FloatTensor);os.makedirs(Path(o).parents[0], exist_ok=True);torchaudio.save(o, data, 16000, encoding="PCM_S", bits_per_sample=16)',
        setup=
        f"import os; from pathlib import Path;import torchaudio;torchaudio.set_audio_backend('soundfile');import torch; DATA = {DATA};OUT_DATA={OUT_DATA}"
    ...
    runner.timeit(
        "Soundfile-i16",
        stmt=
        'for f, o in zip(DATA, OUT_DATA): data=soundfile.read(f, dtype=np.float32)[0];os.makedirs(Path(o).parents[0], exist_ok=True);soundfile.write(o, data, 16000, subtype="PCM_16")',
        setup=
        f"import os; from pathlib import Path;import soundfile;import numpy as np; DATA = {DATA};OUT_DATA={OUT_DATA}"
    )
...
def benchmark_i16_as_f32_22050(runner):
    ...
    runner.timeit(
        "TorchAudio-f32-as-i16-Sox-22050",
        stmt=
        'for f, o in zip(DATA, OUT_DATA): data=torchaudio.load(f)[0].type(torch.FloatTensor);os.makedirs(Path(o).parents[0], exist_ok=True);torchaudio.save(o,  data, 22050, encoding="PCM_S", bits_per_sample=16)',
        setup=
        f"import os; from pathlib import Path;import torchaudio;torchaudio.set_audio_backend('sox_io');import torch; DATA = {DATA};OUT_DATA={OUT_DATA}"
    )
    runner.timeit(
        "TorchAudio-f32-as-i16-Soundfile-22050",
        stmt=
        'for f, o in zip(DATA, OUT_DATA): data=torchaudio.load(f)[0].type(torch.FloatTensor);os.makedirs(Path(o).parents[0], exist_ok=True);torchaudio.save(o, data, 22050, encoding="PCM_S", bits_per_sample=16)',
        setup=
        f"import os; from pathlib import Path;import torchaudio;torchaudio.set_audio_backend('soundfile');import torch; DATA = {DATA};OUT_DATA={OUT_DATA}"
    ...
    # )
    runner.timeit(
        "Soundfile-f32-as-i16-22050",
        stmt=
        'for f, o in zip(DATA, OUT_DATA): data=soundfile.read(f, dtype=np.float32)[0];os.makedirs(Path(o).parents[0], exist_ok=True);soundfile.write(o, data, 22050, subtype="PCM_16")',
        setup=
        f"import os; from pathlib import Path;import soundfile;import numpy as np; DATA = {DATA};OUT_DATA={OUT_DATA}"
    )
...
def benchmark_i16_as_f32_44100(runner):
    ...
    runner.timeit(
        "TorchAudio-f32-as-i16-Sox-44100",
        stmt=
        'for f, o in zip(DATA, OUT_DATA): data=torchaudio.load(f)[0].type(torch.FloatTensor);os.makedirs(Path(o).parents[0], exist_ok=True);torchaudio.save(o,  data, 44100, encoding="PCM_S", bits_per_sample=16)',
        setup=
        f"import os; from pathlib import Path;import torchaudio;torchaudio.set_audio_backend('sox_io');import torch; DATA = {DATA};OUT_DATA={OUT_DATA}"
    )
    runner.timeit(
        "TorchAudio-f32-as-i16-Soundfile-44100",
        stmt=
        'for f, o in zip(DATA, OUT_DATA): data=torchaudio.load(f)[0].type(torch.FloatTensor);os.makedirs(Path(o).parents[0], exist_ok=True);torchaudio.save(o, data, 44100, encoding="PCM_S", bits_per_sample=16)',
        setup=
        f"import os; from pathlib import Path;import torchaudio;torchaudio.set_audio_backend('soundfile');import torch; DATA = {DATA};OUT_DATA={OUT_DATA}"
    ...
    # )
    runner.timeit(
        "Soundfile-f32-as-i16-44100",
        stmt=
        'for f, o in zip(DATA, OUT_DATA): data=soundfile.read(f, dtype=np.float32)[0];os.makedirs(Path(o).parents[0], exist_ok=True);soundfile.write(o, data, 44100, subtype="PCM_16")',
        setup=
        f"import os; from pathlib import Path;import soundfile;import numpy as np; DATA = {DATA};OUT_DATA={OUT_DATA}"
    )
